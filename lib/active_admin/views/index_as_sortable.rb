  ...
  module Views
    ...
    class IndexAsSortable < ActiveAdmin::Component
      ...
      def build_default_actions(resource)
        links = ''.html_safe
        if controller.action_methods.include?('show') && authorized?(:show, resource)
          links << link_to(I18n.t('active_admin.view'), resource_path(resource), :class => "member_link view_link")
        end
        if controller.action_methods.include?('edit') && authorized?(:edit, resource)
          links << link_to(I18n.t('active_admin.edit'), edit_resource_path(resource), :class => "member_link edit_link")
        end
        if controller.action_methods.include?('destroy') && authorized?(:delete, resource)
